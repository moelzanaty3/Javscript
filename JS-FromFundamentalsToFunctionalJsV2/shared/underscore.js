...
_.each = function (list, callback) {
    ...
    if (Array.isArray(list)) {
        list.forEach((element, index, list) => (callback(element, index, list)));
    } else {
...
_.map = (list, callback) => {
    ...
    const results = [];
    if (Array.isArray(list)) {
        _.each(list, (element, index, list) => (results.push(callback(element, index, list))))
    }
    return results;
}
...
_.filter = (list, callback) => {
    ...
    if (Array.isArray(list)) {
        _.each(list, (element, index) => {
            if (callback(element, index, list)) results.push(element)
        })
    }
    return results;
}
